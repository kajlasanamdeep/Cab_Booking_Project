...
async function register(req, res) {
    try {
        ...
        if (req.file) {
            await new Model.Users.UserImage({
        ...
        let data = { user: user, userImage: req.file.path, accessToken: Token };
...
async function updateImage(req, res){
    try {
        let user = req.loggedUser;
        if (req.file) {
            await Model.Users.UserImage.findOneAndUpdate({userID:mongoose.Types.ObjectId(user._id)},{Image: req.file.path,isUploaded:true},{upsert:true});
